...
export function EditProduct({
  ...
  const clearExistingImage = (id: string) => {
    setExistingImage((prev) => prev?.filter((image: string) => image !== id));
  };
  ...
  const handleEdit = async (data: FieldValues) => {
    ...
    if (existingImage.length > 0) {
      data.existingImages = existingImage;
    }
    ...
    const loadingId = toast.loading("Editing product...");
    const result = await editProduct(formData, id as string);

    if (result.success) {
      toast.success("Product edited successfully", { id: loadingId });
      setOpen(false);
    } else {
      toast.error("Failed to edit product", { id: loadingId });
    }
  };
              ...
              <FormItem>
                ...
                <FormControl>
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectGroup>
                        <SelectItem value="electronis">Electronics</SelectItem>
